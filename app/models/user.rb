...
class User < ActiveRecord::Base
  validates_presence_of :username, :enc_password, :on => :create, :message => "can't be blank"
  validates_uniqueness_of :username, :on => :create, :message => "must be unique"
  validates_length_of :username, :within => 5..40, :on => :create, :message => "must be present"
  validates_length_of :password, :within => 8..40, :on => :create, :message => "must be present"
  validates_confirmation_of :enc_password, :on => :create, :message => "should match confirmation"
